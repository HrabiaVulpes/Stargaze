...
public class Galaxy {
   ...
   private void optimize() {
      ...
      starSystems.forEach(starSystem -> {
         starSystem.setX(starSystem.getX() - minX + 20);
         starSystem.setY(starSystem.getY() - minY + 20);
      });
   }

   public Set<StarSystem> getSystemsByOwner(Player player) {
      return starSystems.stream().filter(system -> system.getOwner() == player).collect(Collectors.toSet());
   }
